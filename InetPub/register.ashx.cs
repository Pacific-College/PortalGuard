...
namespace PortalGuard
{
  public class RegistrationHandler : IHttpHandler, IRequiresSessionState
  {
    ...
    private int PreCreateUserAccount(ref HttpRequest req, ref HttpResponse resp)
    {
      ...
      if (0 == String.Compare(program, "bsn-pre", true) || 0 == String.Compare(program, "mc cert", true) || 0 == String.Compare(program, "dac upgrade", true) || 0 == String.Compare(program, "dacm upgrade", true))
      {
        SetFormField(ref req, "Group4", String.Format("CN=Blackboard Learn Access,OU=Security Groups{0}", baseDN));
      }
      return PGAPI_RC_NOERROR;
    }
